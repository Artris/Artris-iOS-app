...
//    }
    ...
    func worldPosition(fromScreenPosition position: CGPoint, objectPosition: float3?, infinitePlane: Bool = false) -> (position: float3, planeAnchor: ARPlaneAnchor?, isOnPlane: Bool)? {
        ...
        */
        ...
        let featurePosition = featureHitTestResult?.position // position of the hit test result

        /*
         3. If desired or necessary (no good feature hit test result): Hit test
            against an infinite, horizontal plane (ignoring the real world).
        */
        if infinitePlane || featurePosition == nil { // if the second option didnt work (no good feature position)
            if let objectPosition = objectPosition, //objectPosition != null: ie it has been placed
                let pointOnInfinitePlane = hitTestWithInfiniteHorizontalPlane(position, objectPosition) {
                return (pointOnInfinitePlane, nil, true) //the intersection  point between the vector to the desired position and a horizontal plane intersection the object
            }
        }
        ...
        */
        if let featurePosition = featurePosition { //if the second option worked featurePosition != Null
    ...
    func hitTestRayFromScreenPosition(_ point: CGPoint) -> HitTestRay? {
        ...
        let screenPosOnFarClippingPlane = unprojectPoint(positionVec) // if z is one: the point is placed on the far clipping plane
 
        let rayDirection = simd_normalize(screenPosOnFarClippingPlane - cameraPos) //vector pointing from camera node to position on far clipping plane
        return HitTestRay(origin: cameraPos, direction: rayDirection)
    }

    func hitTestWithInfiniteHorizontalPlane(_ point: CGPoint, _ pointOnPlane: float3) -> float3? {
        guard let ray = hitTestRayFromScreenPosition(point) else { return nil } //get ray from the camera node to the position on the far clipping plane
    ...
    func hitTestWithFeatures(_ point: CGPoint, coneOpeningAngleInDegrees: Float, minDistance: Float = 0, maxDistance: Float = Float.greatestFiniteMagnitude, maxResults: Int = 1) -> [FeatureHitTestResult] {
        ...
        let results = features.points.flatMap { featurePosition -> FeatureHitTestResult? in
            let originToFeature = featurePosition - ray.origin //ray.origin is the position of the camera node

            let crossProduct = simd_cross(originToFeature, ray.direction) //find the cross product between the ray and the feature point vector. This is the normal from the feature point to the ray
            let featureDistanceFromResult = simd_length(crossProduct) //find the length of the cross product to find the shortest distance between the feature point and the ray
            
            // the projection of the originToFeature ray unto the ray plus the vector from the root to the origin -> hit Test
            ...
            let angleBetweenRayAndFeature = acos(simd_dot(ray.direction, originToFeatureNormalized)) //find the angle between the ray and the feature
        ...
        // Sort the results by feature distan ce to the ray origin.
        let sortedResults = results.sorted { $0.distanceToRayOrigin < $1.distanceToRayOrigin }

		let remainingResults = maxResults > 0 ? Array(sortedResults.prefix(maxResults)) : sortedResults //return the shortest distance within the array

        return Array(remainingResults) //return an array with the hit test with the shortest length within the specified range
    }
    ...
    func hitTestWithFeatures(_ point: CGPoint) -> [FeatureHitTestResult] {
        guard let features = session.currentFrame?.rawFeaturePoints,
            let ray = hitTestRayFromScreenPosition(point) else {
                return [] //find ray from origin to hit
        }
